...
def getWindSpeed(startTime:str, endTime:str, connection: epm.EpmConnection):
    ...
    with open(models.getpathSuporte() + "P11 -windspeed_EPM.txt") as f:
        pyrp = json.load(f)
    ...
    prefixos = sorted(set(col.split('-')[0] for col in df_measures.columns))
    df_measures.columns = prefixos
    

    # combined_df = pd.concat([combined_df, df_measures], axis=1)    
    # combined_df[combined_df.values<0]=0
    # combined_df = combined_df.apply(lambda row: row.fillna(row.mean()), axis=1)
    # prefixos = sorted(set(col.split('-')[0] for col in combined_df.columns))
    # combined_df.columns = prefixos
